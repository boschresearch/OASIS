...
def preprocess_input_ddp(opt, data, device):
    data['label'] = data['label'].long().to(device)
    label_map = data['label']
    ...
    nc = opt.semantic_nc
    if opt.gpu_ids != "-1":
        input_label = torch.empty(bs, nc, h, w,  device = device).zero_()
    else:
        input_label = torch.FloatTensor(bs, nc, h, w).zero_()
...
class OASIS_model_ddp():
    def __init__(self, opt, ema = True):
        #super(OASIS_model, self).__init__()
        self.opt = opt
        #--- generator and discriminator ---
        self.netG = generators.OASIS_Generator(opt)
